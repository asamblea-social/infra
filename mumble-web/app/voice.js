...
class VoiceHandler extends Writable {
  constructor (client, settings) {
    ...
    this._client = client
    this._id_debug = Math.random()
    console.log("VoiceHandler constructor ID:", Math.random(), "client:", client ? client._username : null)
  ...
  _getOrCreateOutbound () {
    ...
    if (!this._outbound) {
      ...
      console.log("OUTBOUND created by ID:", this._id_debug, "client:", this._client ? this._client._username : null)
      this._id_debug = Math.random()
      ...
      if (this._client instanceof WorkerBasedMumbleClient) {
        ...
        // Direct (non-worker) client: createVoiceStream() expects each write to be
        // exactly one valid Opus frame duration (960 samples = 20ms at 48kHz).
        // The microphone ScriptProcessorNode delivers 4096-sample buffers, so we
        // must rechunk to the packet size Opus requires before it reaches the encoder.
