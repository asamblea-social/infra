...
class GlobalBindings {
  constructor (config) {
    ...
    this.connect = (username, host, port, tokens = [], password, channelName = "") => {
      ...
      // TODO: token
      this.webrtcConnector.connect(`wss://${host}:${port}/mumble`, {
        username: username,
        password: password,
        webrtc: this.webrtc ? {
          enabled: true,
      ...
      user.on('update', (actor, properties) => {
        ...
        stream.on('data', data => {
          if (this.webrtc) {
            ...
            userNode.write(Buffer.from(data.pcm.buffer, data.pcm.byteOffset, data.pcm.byteLength))
          }
        }).on('end', () => {
...
async function main() {
  ...
  try {
    const userMedia = await initVoice(data => {
      ...
      if (!ui.client) {
        if (voiceHandler) console.log("DESTROYING voiceHandler because ui.client is null")
        if (voiceHandler) {
          voiceHandler.end()
        }
        voiceHandler = null
      } else if (voiceHandler) {
        console.log("WRITING to voiceHandler", data.length)
